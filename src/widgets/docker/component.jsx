...
export default function Component({ service }) {
  ...
  const { data: statusData, error: statusError } = useSWR(
    `/admin/api/docker/status/${widget.container}/${widget.server || ""}`,
  );

  const { data: statsData, error: statsError } = useSWR(`/admin/api/docker/stats/${widget.container}/${widget.server || ""}`);
